...
test('Get Recipe | chefkoch', async () => {
  const recipe = await getRecipe(
    'https://www.chefkoch.de/rezepte/583381157461209/Ravioli-mit-gebratenen-Garnelen-und-Spargel-in-Kraeutersahne.html',
  );
  console.info(recipe);
...
test('Get Recipe | juliasalbum', async () => {
  const recipe = await getRecipe(
    'https://juliasalbum.com/spinach-ravioli-cream-sauce-ricotta-cheese-filling-cherry-tomatoes-recipe/',
  );
  console.info(recipe);
